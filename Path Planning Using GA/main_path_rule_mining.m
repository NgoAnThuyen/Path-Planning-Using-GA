...
dims = [1 35];
definput = {'400','100','60','0.9','0.35','1'};
...
while ~Terminal,
    ...
    [a,b]=check_node(parent(1,:));
    Pool_new =  AddToPool(b,map);
    Pool.path = [Pool.path ; Pool_new.path ];
    Pool.dis = [Pool.dis ; Pool_new.dis ];
      ExtractPool(parent(pop_index,:),map,Pool);
...
xlabel('Generation');
ylabel('Fitness');
[n_best, path_best] =  check_node(par(bestchrom,:))
plotResult
